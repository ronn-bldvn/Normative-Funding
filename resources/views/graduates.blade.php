...
@section('page_title')
    <h2 id="dashboardTitle"
        class="text-lg lg:text-2xl font-['Bricolage_Grotesque'] font-extrabold text-black leading-tight break-words">
        {{ $dynamic_title }}
    </h2>
@endsection
...
@section('content')
    ...
    const DEGREE_PREFIXES = [
        ...
        { pattern: /^Doctor of Philosophy\s*/i,           short: 'PhD'   },
    ];
    ...
    // ─── Colour Palette ────────────────────────────────────────────────────────
    const PALETTE = [
        '#16A34A','#22C55E','#86EFAC','#4ADE80','#15803D',
        '#65A30D','#84CC16','#A3E635','#FACC15','#FB923C',
        '#F97316','#EF4444',
    ];
    ...
    function renderHeadcountDonut(data) {
        ...
        // shortLabels used in legend + slices; full names used in tooltips
        const shortLabels = data.labels.map(abbreviateCollege);
        const colors      = PALETTE.slice(0, data.labels.length);

        Plotly.newPlot('headcountDonut', [{
            type: 'pie',
            labels: shortLabels,
            values: data.values,
            customdata: data.labels,        // full name shown on hover
            hole: 0.58,
    ...
    function renderRankingBar(data) {
        ...
        // Sort ascending so highest value sits at the top of the horizontal bar
        ...
        Plotly.newPlot('rankingBar', [{
            ...
            orientation: 'h',
            x: rows.map(d => d.val),
            y: rows.map(d => d.short),       // abbreviated on axis
            customdata: rows.map(d => d.full), // full name in tooltip
            text: rows.map(d => d.val),
            textposition: 'outside',
            cliponaxis: false,
            marker: {
                color: rows.map(d => d.highlight ? '#F59E0B' : '#16A34A'),
