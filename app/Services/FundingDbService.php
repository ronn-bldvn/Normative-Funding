...
class FundingDbService
{
    ...
    public function readIncome(string $year): array
    {
        ...
        $grandRows = DB::table('fund_report_income_lines')
        ...
        $details = DB::table('fund_report_income_lines')
            ->where('fund_report_id', $rid)
    ...
    private function readAE(string $year, string $type): array
    {
        ...
        $totals = DB::table('fund_report_ae_lines')
            ->where('fund_report_id', $rid)
            ->where('is_total', true)
            ->first();
        ...
        $lines = DB::table('fund_report_ae_lines')
            ->where('fund_report_id', $rid)
